...
class SocketService {
  ...
  addEvents(token) {
    ...
    this.socket.on(eventTypes_1.INIT, formatEvt(message => {
      console.log(`---------- message ----------`);
      console.log(message);
    }));
    this.socket.on(eventTypes_1.CMD_ERR, callAction());
  }
  onConnection(token, message) {
  }
  emit(type, data) {
    if (!this.socket) return console.error(`Socket not connected, cannot emit socket event!`);
    if (!type) return console.error(`Event type is missing, cannot emit socket event!`, type);
...
const reducer = (state, action) => {
  switch (action.type) {
    case eventTypes_1.ON_CONNECTED:
      {
...
const useSocketReducer = (customReducer, customInitialState, config) => {
  const socketReducer = isFunc(customReducer) ? joinReducers(reducer, customReducer) : reducer;
  const [state, dispatch] = useReducer(socketReducer, deepMerge(initialState, customInitialState));
  return useMemo(() => ({
    state,
    dispatch,
    config
...
const WebSocketProvider = props => {
  ...
  const websocket = useSocketReducer(reducer || noOp, initialState || noOpObj, config || noOpObj);
  useEffect(() => {
...
export { WebSocketHoc, WebSocketProvider, useWebSocket };
