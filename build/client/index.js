...
const toggleIsRunning = ({ isRunning, name }) => {
  getDispatch()({
...
const callAction = (instance, event, action) => {
  ...
  return data => {
    ...
    eventName === 'init' &&
      (instance.commands = jsutils.get(message, 'data.commands'))
...
class SocketService {
  ...
  addEvents(token) {
    if (!this.socket) return
    Object.entries(jsutils.get(this.config, 'events', jsutils.noOpObj)).map(
      ([name, action]) => {
        const namCaps = jsutils.snakeCase(name).toUpperCase()
        if (namCaps === 'ALL') return
        const eventType = `${eventTypes_2}:${namCaps}`
        jsutils.isFunc(action) &&
          !frozenEvents[namCaps] &&
...
const sockrReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case frozenEvents.SET_ID: {
      ...
      return !action.id ? state : { ...state, ...updates }
    }
    ...
    case frozenEvents.ADD_PEER: {
      return !action.peers ? state : { ...state, peers: action.peers }
    }
...
const joinReducers = (sockrReducer, customReducer) => {
  ...
  _JOINED_REDUCERS = (state, action) => {
    ...
    return jsutils.isFunc(customReducer)
...
const useSockrReducer = (customReducer, customInitialState) => {
  const [state, dispatch] = React.useReducer(
    ...
    jsutils.deepMerge(initialState$1, customInitialState)
  )
...
const MemoChildren = React__default['default'].memo(props =>
  React__default['default'].createElement(
